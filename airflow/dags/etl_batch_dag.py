...
with dag:
    # Start task
    etl_start = BashOperator(task_id="etl_start", bash_command='echo "Starting ETL batch processing at $(date)"')
    ...
    with TaskGroup("load_dimensional_data") as load_dims:
        load_products = SparkSubmitOperator(
            ...
            conf={"spark.executor.memory": "4g", "spark.executor.cores": "2", "spark.shuffle.partitions": "200"},
        )
    ...
    # End task
    etl_end = BashOperator(task_id="etl_end", bash_command='echo "ETL batch processing completed at $(date)"')
