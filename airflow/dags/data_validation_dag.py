...
def record_validation_status(status: str, details: str) -> None:
    ...
    with engine.connect() as conn:
        conn.execute(
            ...
            """),
            {"check_name": "daily_validation", "status": status, "record_count": 0, "timestamp": datetime.utcnow()},
        )
        conn.commit()
...
with dag:
    # Start task
    validate_start = BashOperator(task_id="validate_start", bash_command='echo "Starting data validation at $(date)"')

    # Validation tasks in parallel
    with TaskGroup("validate_staging_tables") as validate_tables:
        validate_orders = PythonOperator(task_id="validate_orders", python_callable=validate_orders_table)

        validate_deliveries = PythonOperator(task_id="validate_deliveries", python_callable=validate_deliveries_table)

        validate_transactions = PythonOperator(
            task_id="validate_transactions", python_callable=validate_transactions_table
        )

    # Create validation summary
    validation_summary = BashOperator(
        task_id="validation_summary", bash_command='echo "All validations completed successfully"'
    )
